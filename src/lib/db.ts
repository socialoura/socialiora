...
export async function getPricing() {
  try {
    ...
    if (result.rows.length > 0) {
      return result.rows[0].data as { instagram: Array<{ followers: string; price: string }>; tiktok: Array<{ followers: string; price: string }>; twitter?: Array<{ followers: string; price: string }>; instagram_likes?: Array<{ followers: string; price: string }>; tiktok_views?: Array<{ followers: string; price: string }> };
    }
    return null;
  } catch (error) {
...
export async function setPricing(data: { instagram: Array<{ followers: string; price: string }>; tiktok: Array<{ followers: string; price: string }>; twitter?: Array<{ followers: string; price: string }>; instagram_likes?: Array<{ followers: string; price: string }>; tiktok_views?: Array<{ followers: string; price: string }> }) {
  try {
    await sql`
...
export async function getFunnelDefaults() {
  try {
    const result = await sql`
      SELECT data FROM pricing WHERE id = 'funnel-defaults'
    `;
    if (result.rows.length > 0) {
      return result.rows[0].data;
    }
...
export async function setFunnelDefaults(data: Record<string, number>) {
  try {
    await sql`
      INSERT INTO pricing (id, data) 
      VALUES ('funnel-defaults', ${JSON.stringify(data)}::jsonb)
      ON CONFLICT (id) 
      DO UPDATE SET data = ${JSON.stringify(data)}::jsonb, updated_at = CURRENT_TIMESTAMP
    `;
  } catch (error) {
    console.error('Error setting funnel defaults:', error);
    throw error;
  }
}
...
export async function getPopularPack(platform: 'instagram' | 'tiktok' | 'twitter' | 'instagram_likes' | 'tiktok_views' | 'tiktok_likes' | 'youtube_views' | 'linkedin_followers'): Promise<string | null> {
  try {
    const result = await sql`
...
export async function setPopularPack(platform: 'instagram' | 'tiktok' | 'twitter' | 'instagram_likes' | 'tiktok_views' | 'tiktok_likes' | 'youtube_views' | 'linkedin_followers', followers: string): Promise<void> {
  try {
