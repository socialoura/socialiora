...
function CheckoutPaymentForm({ amount, currency, email, acceptedTerms, lang, onSuccess, onPaymentIntentId, onBeforePayment, i18n }: CheckoutPaymentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    posthog.capture('instagram2_step4_payment_attempted', { payment_method_type: 'card', target_platform: 'instagram', source: 'instagram-2_funnel' });
    onBeforePayment?.();
    ...
    if (error) {
      posthog.capture('instagram2_step4_payment_failed', { error_code: error.code || 'unknown', error_message: error.message || 'unknown', target_platform: 'instagram', source: 'instagram-2_funnel' });
  ...
  const handleExpressConfirm = async () => {
    ...
    posthog.capture('instagram2_step4_payment_attempted', { payment_method_type: 'express', target_platform: 'instagram', source: 'instagram-2_funnel' });
    onBeforePayment?.();
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex items-start gap-3 p-4 rounded-xl bg-gray-800/30 border border-gray-700/50">
        <input
          ...
          readOnly
          className="mt-0.5 h-5 w-5 text-pink-500 focus:ring-pink-500/50 border-gray-600 rounded bg-gray-900 shrink-0 cursor-not-allowed"
        />
...
export default function CheckoutSummary({ lang }: CheckoutSummaryProps) {
  ...
  useEffect(() => {
    ...
    posthog.capture('instagram2_step4_checkout_viewed', {
      final_price: totalPrice,
      final_service: primaryService?.type || 'unknown',
      target_platform: 'instagram',
      source: 'instagram-2_funnel',
    });
                      ...
                      <CheckoutPaymentForm
                        ...
                        onSuccess={async () => {
                          ...
                          try {
                            ...
                            const source = getPurchaseSource(window.location.pathname, 'APP_FUNNEL_INSTA2');
