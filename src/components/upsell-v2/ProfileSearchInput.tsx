...
import { Loader2, Search, AlertCircle, Instagram, ChevronRight, Users } from 'lucide-react';
...
export default function ProfileSearchInput({ lang }: ProfileSearchInputProps) {
  ...
  const whiteHat = {
    ...
    de: {
      title: 'ERWEITERN SIE IHR PUBLIKUM',
      subtitle: 'Das Marketing-Tool, das Ihre Sichtbarkeit steigert. Geben Sie Ihr öffentliches Profil ein, um loszulegen. (Wir fragen niemals nach Ihrem Passwort).',
      placeholder: 'username',
      cta: 'Starten',
    },
    es: {
      title: 'HAGA CRECER SU AUDIENCIA',
      subtitle: 'La herramienta de marketing diseñada para impulsar su visibilidad. Ingrese su perfil público para comenzar. (Nunca le pediremos su contraseña).',
      placeholder: 'username',
      cta: 'Empezar',
    },
  };

  const wh = whiteHat[lang];
  ...
  const handleSearch = async (searchUsername: string) => {
    ...
    posthog.capture('instagram2_step1_search_initiated', { target_platform: 'instagram', source: 'instagram-2_funnel' });
    ...
    try {
      ...
      posthog.capture('instagram2_step1_search_failed', { error_reason: errorMsg, target_platform: 'instagram', source: 'instagram-2_funnel' });
  ...
  const handleSelectProfile = () => {
    ...
    posthog.capture('instagram2_step1_profile_found', { follower_count: searchResult.followersCount, target_platform: 'instagram', source: 'instagram-2_funnel' });
    ...
    <div className="w-full max-w-2xl mx-auto mt-2 sm:mt-12 min-h-[60vh]">
      ...
      <div className="flex items-center justify-center mt-6 sm:mt-8">
        <div className="inline-flex items-center gap-2 rounded-xl bg-white/5 border border-white/10 px-3 py-2" aria-label="Trustpilot rating 4.8 out of 5">
          <span className="text-sm font-black text-white">4.8/5</span>
