...
function CheckoutPaymentForm({ amount, currency, email, acceptedTerms, setAcceptedTerms, lang, onSuccess, onPaymentIntentId, onBeforePayment, i18n }: CheckoutPaymentFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    posthog.capture('tiktok2_step4_payment_attempted', { payment_method_type: 'card', target_platform: 'tiktok', source: 'tiktok-2_funnel' });
    onBeforePayment?.();
    ...
    if (error) {
      posthog.capture('tiktok2_step4_payment_failed', { error_code: error.code || 'unknown', error_message: error.message || 'unknown', target_platform: 'tiktok', source: 'tiktok-2_funnel' });
  ...
  const handleExpressConfirm = async () => {
    ...
    posthog.capture('tiktok2_step4_payment_attempted', { payment_method_type: 'express', target_platform: 'tiktok', source: 'tiktok-2_funnel' });
    onBeforePayment?.();
...
export default function TiktokCheckoutSummary({ lang }: CheckoutSummaryProps) {
  ...
  useEffect(() => {
    ...
    posthog.capture('tiktok2_step4_checkout_viewed', {
      final_price: totalPrice,
      final_service: primaryService?.type || 'unknown',
      target_platform: 'tiktok',
      source: 'tiktok-2_funnel',
    });
                          ...
                          try {
                            ...
                            const orderRes = await fetch('/api/orders/create', {
                              ...
                              body: JSON.stringify({
                                ...
                                orderSource: 'APP_FUNNEL_TIKTOK2',
                                funnelData,
                            ...
                            const source = getPurchaseSource(window.location.pathname, 'APP_FUNNEL_TIKTOK2');
